...
var CORRUPTION_COLORS = {
  "chaos-corruptions-khorne-corruption": "#FF2400",
  // 코른
  "chaos-corruptions-nurgle-corruption": "#324C09",
  // 너글
  "chaos-corruptions-tzeentch-corruption": "#8A2BE2",
  // 젠취
  "chaos-corruptions-slaanesh-corruption": "#DA70D6"
  // 슬라네쉬
};
var updateCorruptionVisuals = (token, controlled) => {
  ...
  if (highestCorruption) {
    const alpha = Math.min(0.1 * highestCorruption.level, 0.7);
    const bgColor = hexToRgba(highestCorruption.color, alpha);
    corruptionLayer.style.backgroundColor = bgColor;
    console.log(
      `${config_default.MODULE_ID} | Applied corruption visual: ${highestCorruption.slug} level ${highestCorruption.level}`
    );
  } else {
    corruptionLayer.style.removeProperty("background-color");
  }
};
var initCorruptionLayer = () => {
  ...
  corruptionLayer.style.zIndex = "60";
  corruptionLayer.style.transition = "background-color 0.5s ease";
  console.log(`${config_default.MODULE_ID} | Creating corruption visualization layer`);
